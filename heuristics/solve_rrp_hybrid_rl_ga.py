import numpy as np
from heuristics.hybrid_rl_ga import HybridRLGA
from heuristics.residual_packing import residual_pack_repair
from utils import timeit, safe_div

def solve_rrp_hybrid_rl_ga(X, K, k_t, delta_bar, w, lambda_penalty, theta1, theta2, theta3, P1, P2, P3, seed):
    """混合强化学习与遗传算法求解RRP问题"""
    ...
    with timeit() as timer:
        best_individual, best_reward = algorithm.run(state_space)
...
def solve_rrp_neural_rl(X, K, k_t, delta_bar, w, lambda_penalty, theta1, theta2, theta3, P1, P2, P3, seed, state_dim=10, action_dim=10, learning_rate=0.001):
    """基于神经网络的强化学习求解RRP问题"""
    ...
    # 运行算法
    with timeit() as timer:
        rewards = algorithm.run(state_space, episodes=1000)
...
# 导出函数
__all__ = ["solve_rrp_hybrid_rl_ga", "solve_rrp_neural_rl", "solve_rrp_es_based_rrp"]